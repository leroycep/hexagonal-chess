...
pub const Renderer = struct {
    ...
    pub fn pushRect(self: *Renderer, pos: Vec2f, size: Vec2f, color: platform.Color, rot: f32) void {
        ...
        const top_left = vec2f(-size.x() / 2, -size.y() / 2).rotate(rot).add(pos);
        const top_right = vec2f(size.x() / 2, -size.y() / 2).rotate(rot).add(pos);
        const bot_left = vec2f(-size.x() / 2, size.y() / 2).rotate(rot).add(pos);
        const bot_right = vec2f(size.x() / 2, size.y() / 2).rotate(rot).add(pos);
    ...
    pub fn pushFlatHexagon(self: *Renderer, pos: Vec2f, radius: f32, color: platform.Color, radians: f32) void {
        ...
        const hex_points = [6]Vec2f{
            Vec2f.init(radius, 0).rotate(radians + 0 * 60.0 * DEG_TO_RAD).add(pos),
            Vec2f.init(radius, 0).rotate(radians + 1 * 60.0 * DEG_TO_RAD).add(pos),
            Vec2f.init(radius, 0).rotate(radians + 2 * 60.0 * DEG_TO_RAD).add(pos),
            Vec2f.init(radius, 0).rotate(radians + 3 * 60.0 * DEG_TO_RAD).add(pos),
            Vec2f.init(radius, 0).rotate(radians + 4 * 60.0 * DEG_TO_RAD).add(pos),
            Vec2f.init(radius, 0).rotate(radians + 5 * 60.0 * DEG_TO_RAD).add(pos),
        };
